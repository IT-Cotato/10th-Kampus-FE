import { getStudentVerificationDetail } from '@/apis/admin/getStudentVerificationsDetail.api';
import {
...
export const StudentVerifications = () => {
  ...
  const { data: studentVerificationData, isLoading } = useQuery({
    queryKey: [QUERY_KEYS.ADMIN_GET_VERIFICATIOIN, verificationRecordId],
    queryFn: () =>
      getStudentVerificationDetail({
  ...
  const { mutate: approveVerification } = useMutation({
    mutationFn: (id) =>
      postApproveStudentVerifications({ verificationRecordId: id }),
    onSuccess: () => {
      navigate(-1);
    },
    onError: (error) => {
  ...
  const handleClickSave = (verificationRecordId) => {
    if (selectedStatus === 'approve') {
      ...
      rejectVerification({ verificationRecordId, rejectionReason: reason });
    }
  };
        ...
        <div className="grid items-center w-full h-full grid-cols-2 gap-5 p-8 bg-white rounded-2xl">
          ...
          <div className="flex flex-col items-center justify-center gap-4 text-center">
            <h1 className="text-subTitle text-neutral-title">
              학교: {studentVerificationData.universityId}
            </h1>
